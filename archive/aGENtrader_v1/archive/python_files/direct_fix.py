...
def apply_direct_fix():
    ...
    try:
        ...
        # First check if the data_integrity.py exists
        if not os.path.exists("agents/data_integrity.py"):
            logger.error("agents/data_integrity.py not found")
            return False
...
def check_smart_backtest_runner():
    """Check if we can find/fix smart-backtest-runner.sh"""
    if not os.path.exists("smart-backtest-runner.sh"):
        logger.warning("smart-backtest-runner.sh not found")
        return False
    
    try:
        with open("smart-backtest-runner.sh", "r") as f:
            content = f.read()
        
        # Check if this script is already called in the runner
        if "direct_fix.py" in content:
            logger.info("direct_fix.py is already called in smart-backtest-runner.sh")
            return True
        
        # Find the best place to inject our fix - before any trading or decision session
        pattern = r"(python3.*\.(py|sh))"
        match = re.search(pattern, content)
        if not match:
            logger.warning("Could not find a suitable location to inject the fix")
            return False
        
        # Inject our fix before the first python script execution
        injection = f"# Apply data integrity fix\npython3 direct_fix.py\n\n{match.group(0)}"
        modified_content = content.replace(match.group(0), injection)
        
        # Write the modified file
        with open("smart-backtest-runner.sh.new", "w") as f:
            f.write(modified_content)
        
        logger.info("Created modified smart-backtest-runner.sh.new with the fix injection")
        return True
    
    except Exception as e:
        logger.error(f"Error checking/fixing smart-backtest-runner.sh: {str(e)}")
        return False
...
def main():
    ...
    if success:
        ...
        # Also check if we should modify the backtest runner
        check_smart_backtest_runner()
        if os.path.exists("smart-backtest-runner.sh.new"):
            print("\nAlso created a modified version of smart-backtest-runner.sh")
            print("To use it, run:")
            print("  mv smart-backtest-runner.sh.new smart-backtest-runner.sh")
            print("  chmod +x smart-backtest-runner.sh")
