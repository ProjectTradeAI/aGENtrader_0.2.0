#!/bin/bash
# Script to deploy and run a hardcoded backtest solution
...
# Welcome message
log "🔨 Deploying Hardcoded Backtest Solution"
log "==================================================="
...
# Step 3: Create directories on EC2 if they don't exist
log "Step 3: Creating directories on EC2..."
ssh -i "$KEY_PATH" -o StrictHostKeyChecking=no "$SSH_USER@$EC2_IP" "mkdir -p $EC2_DIR/logs/backtests $EC2_DIR/results"

# Step 4: Create a hardcoded run script on EC2
log "Step 4: Creating a hardcoded run script on EC2..."

# Create the run script with hardcoded credentials
# Add the actual Python code to the script with proper escaping
ssh -i "$KEY_PATH" -o StrictHostKeyChecking=no "$SSH_USER@$EC2_IP" "cat > $EC2_DIR/run_backtest_hardcoded.sh << 'EOL'
#!/bin/bash
# Hardcoded backtest runner
# Usage: ./run_backtest_hardcoded.sh --symbol BTCUSDT --interval 1h --start_date 2025-04-01 --end_date 2025-04-10 --initial_balance 10000

# Create Python script with hardcoded credentials
cat > backtest_hardcoded.py << 'PYEOF'
#!/usr/bin/env python3
\"\"\"
...
chmod +x backtest_hardcoded.py

# Run the backtest with the provided arguments
...
# Step 5: Create a local run script
log "Step 5: Creating local run script..."

cat > ./run-backtest-hardcoded.sh << 'EOL'
#!/bin/bash
# Local script to run hardcoded backtest on EC2
...
chmod +x ./run-backtest-hardcoded.sh

log_info "✅ Local run script created."
log "The hardcoded backtest solution has been deployed. You can now run backtests with:"
log "   ./run-backtest-hardcoded.sh --symbol BTCUSDT --interval 1h --start 2025-04-01 --end 2025-04-10"