...
LOG_FILE="quick-debug-${TIMESTAMP}.log"
...
echo "🔍 Running Quick Debug on EC2" | tee "$LOG_FILE"
echo "==========================" | tee -a "$LOG_FILE"
...
# Upload quick debug script to EC2
echo "Uploading quick debug script to EC2..." | tee -a "$LOG_FILE"
scp -i "$KEY_PATH" -o StrictHostKeyChecking=no quick-debug.py "$SSH_USER@$EC2_IP:$EC2_DIR/" >> "$LOG_FILE" 2>&1
ssh -i "$KEY_PATH" -o StrictHostKeyChecking=no "$SSH_USER@$EC2_IP" "chmod +x $EC2_DIR/quick-debug.py" >> "$LOG_FILE" 2>&1

# Run quick debug script on EC2
echo "Running quick debug script on EC2..." | tee -a "$LOG_FILE"
ssh -i "$KEY_PATH" -o StrictHostKeyChecking=no "$SSH_USER@$EC2_IP" "cd $EC2_DIR && export PYTHONPATH=\$PWD && DATABASE_URL='$DB_URL' ALPACA_API_KEY='$ALPACA_API_KEY' ALPACA_API_SECRET='$ALPACA_API_SECRET' OPENAI_API_KEY='$OPENAI_API_KEY' python3 quick-debug.py" | tee -a "$LOG_FILE"

echo "✅ Quick debug completed" | tee -a "$LOG_FILE"
