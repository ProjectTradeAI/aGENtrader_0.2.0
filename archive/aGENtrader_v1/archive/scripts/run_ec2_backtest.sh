#!/bin/bash
# Run a backtest on EC2 with the full multi-agent framework
...
if [ -n "$LATEST_RESULT" ]; then
  ...
  # Check if results are valid JSON and display summary
    ...
    if trades:
        ...
        for trade in trades[-3:]:
            if trade.get('type') == 'ENTRY':
                print(f\"  {trade.get('direction', '')} at ${trade.get('price', 0):.2f} ({trade.get('timestamp', '')})\")
            else:
                print(f\"  EXIT {trade.get('direction', '')} at ${trade.get('exit_price', 0):.2f} ({trade.get('timestamp', '')}) - PnL: {trade.get('pnl_percentage', 0):.2f}%\")
